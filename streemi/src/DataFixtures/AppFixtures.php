...
class AppFixtures extends Fixture
{
    ...
    protected function createSubscriptions(ObjectManager $manager, array $users, array &$subscriptions): void
    {
        ...
        foreach ($array as $element) {
            ...
            $abonnement->setDurationInMonths(duration_in_months: $element['duration']);
            ...
            for ($i = 0; $i < random_int(min: 1, max: self::MAX_SUBSCRIPTIONS); $i++) {
                ...
                $randomUser->addCurrentSubscriptionId(currentSubscriptionId: $abonnement);
            }
        }
    }
    ...
    protected function createMedia(ObjectManager $manager, array &$medias): void
    {
        for ($j = 0; $j < self::MAX_MEDIA; $j++) {
            ...
            $media->setLongDescription(long_description: "Longue description $j");
            $media->setShortDescription(short_description: "Short description $j");
            $media->setCoverImage(cover_image: "https://picsum.photos/1920/1080?random=$j");
            $media->setReleaseDate(release_date: new DateTime(datetime: "+7 days"));
    ...
    */
    ...
    public function createPlaylists(ObjectManager $manager, array $users, array &$playlists): void
    {
        foreach ($users as $user) {
            for ($k = 0; $k < self::PLAYLISTS_PER_USER; $k++) {
                ...
                $playlist->setCreatedAt(created_at: new DateTimeImmutable());
                $playlist->setUpdatedAt(updated_at: new DateTimeImmutable());
                $playlist->setUserId(user_id: $user);
                $playlists[] = $playlist;
    ...
    protected function createCategories(ObjectManager $manager, array &$categories): void
    {
        ...
        foreach ($array as $element) {
            ...
            $category->setName(name: $element['nom']);
    ...
    protected function createLanguages(ObjectManager $manager, array &$languages): void
    {
        ...
        foreach ($array as $element) {
            ...
            $language->setName(name: $element['nom']);
    ...
    protected function createComments(ObjectManager $manager, array $medias, array $users): void
    {
        ...
        foreach ($medias as $media) {
            for ($i = 0; $i < random_int(min: 1, max: self::MAX_COMMENTS_PER_MEDIA); $i++) {
                $comment = new Comment();
                $comment->setUserId($users[array_rand(array: $users)]);
                ...
                $comment->setMediaId($media);

                $shouldHaveParent = random_int(0, 5) < 2;
                if ($shouldHaveParent) {
                    $parentComment = new Comment();
                    $parentComment->setUserId($users[array_rand($users)]);
                    ...
                    $parentComment->setMediaId($media);
    ...
    protected function linkSubscriptionToUsers(array $users, array $subscriptions, ObjectManager $manager): void
    {
        foreach ($users as $user) {
            ...
            for ($i = 0; $i < random_int(min: 1, max: self::MAX_SUBSCRIPTIONS_HISTORY_PER_USER); $i++) {
                ...
                $history->setUserId(user_id: $user);
                $history->setSusbscriptionId(susbscription_id: $sub);
                $history->setStartDate(start_date: new DateTimeImmutable());
                $history->setEndDate(end_date: new DateTimeImmutable());
    ...
    protected function addUserPlaylistSubscriptions(ObjectManager $manager, array $users, array $playlists): void
    {
        ...
        foreach ($users as $user) {
            for ($i = 0; $i < random_int(min: 0, max: self::MAX_PLAYLIST_SUBSCRIPTION_PER_USERS); $i++) {
                ...
                $subscription->setUserId(user_id: $user);
                $subscription->setPlaylistId(playlist_id: $playlists[array_rand(array: $playlists)]);
                $subscription->setSubscripedAt(subscriped_at: new DateTimeImmutable());
